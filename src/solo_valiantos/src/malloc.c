...
// Global variables, state of the kernel heap
uint32_t *last_alloc;
uint32_t heap_end;
uint32_t heap_begin;
...
{
    last_alloc = (uint32_t *)((uint32_t)kernel_end + 0x1000); // kernel_end is a pointer to the end of the kernel
    heap_begin = (uint32_t)last_alloc;
    pheap_end = 0x400000; // Example end of page heap
    pheap_begin = pheap_end - (MAX_PAGE_ALIGNED_ALLOCS * 4096);
    heap_end = pheap_begin;
    memset((char *)heap_begin, 0, heap_end - heap_begin);
    pheap_desc = (uint8_t *)malloc(MAX_PAGE_ALIGNED_ALLOCS); // malloc has been defined elsewhere
...
void *malloc(size_t size)
{
    ...
    while ((uint32_t)mem < (uint32_t)last_alloc)
    {
        ...
        // block is not allocated and its size is not big enough,add its size and the sizeof(alloc_t) to the pointer and continue.
        mem += a->size;
        mem += sizeof(alloc_t);
        mem += 4;
    }
